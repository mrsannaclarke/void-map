...
/***** SCHEDULER: run parser every 12 hours *****/
function installTimeTrigger() {
  ...
  // Create a fresh 12-hour trigger
...
function doGet(e) {
  ...
  for (let i = 1; i < vals.length; i++) {
    ...
    const exists = arr.some(g =>
      g.name === display && g.social === social && g.ticket === ticket
    );
    if (!exists) {
      arr.push({ name: display, social: social, ticket: ticket });
    }
  }

  // Build response
  const spaces = {};
  Object.keys(groupsBySpace).forEach(key => {
    ...
    // Status rules:
    // - If buyoutOnly & any booking → full
    // - Else if any group has Buy-out ticket → full
    // - Else status empty (front-end will handle partials and capacity math with map JSON)
    let status = '';
    if ((flags.buyoutOnly && any) || hasBuyout) status = 'full';

    // Back-compat single fields (name/social) = first group if present
